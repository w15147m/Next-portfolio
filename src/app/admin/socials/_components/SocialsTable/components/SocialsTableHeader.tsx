...
export default function SocialsTableHeader({headers}:headersInterface) {
...
{headers.map((header, index, array)=>(
        ...
        <TableCell key={index} isHeader className={`${headerCellClass } ${ index === array.length - 1 &&  'text-right'}`}>
          {header}
        </TableCell>
) )}
